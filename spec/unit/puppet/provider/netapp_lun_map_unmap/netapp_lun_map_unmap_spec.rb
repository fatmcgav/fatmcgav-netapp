...
describe Puppet::Type.type(:netapp_lun_map_unmap).provider(:netapp_lun_map_unmap) do

  device_conf =  YAML.load_file(my_fixture('device_conf.yml'))
  before :each do
    Facter.stubs(:value).with(:url).returns(device_conf['url'])
    described_class.stubs(:suitable?).returns true
    Puppet::Type.type(:netapp_lun_map_unmap).stubs(:defaultprovider).returns described_class
  end

  let :lun_map do
    Puppet::Type.type(:netapp_lun_map_unmap).new(
