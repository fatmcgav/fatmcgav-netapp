...
describe Puppet::Type.type(:netapp_igroup_add_remove_initiator).provider(:netapp_igroup_add_remove_initiator) do
  ...
  before :each do
    ...
    Puppet::Type.type(:netapp_igroup_add_remove_initiator).stubs(:defaultprovider).returns described_class
  end
  ...
  let :add_initiator do
    Puppet::Type.type(:netapp_igroup_add_remove_initiator).new(
  ...
  describe "remove initiator from igroup" do
    it ":should be able to remove initiator from igroup" do
      #remove_initiator.provider.should be_exists
